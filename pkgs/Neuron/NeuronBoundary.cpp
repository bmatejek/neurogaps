...
supervoxel_one(NULL),
supervoxel_two(NULL),
supervoxel_one_index(-1),
supervoxel_two_index(-1),
maximum(RN_UNKNOWN),
mean(RN_UNKNOWN),
median(RN_UNKNOWN),
minimum(RN_UNKNOWN),
...
~NeuronBoundary(void)
{
   ...
   if (data) data->RemoveBoundary(this);
}
...
RandomWalk(RNBoolean drunkards_walk) const
{
   ...
   *extp = '\0';
   ...
   if (cache_fp) {
      ...
      rn_assertion(nboundaries == data->NBoundaries());

      RNScalar value;
      if (drunkards_walk) {
         drunkard_walk_features = new RNScalar[data->NBoundaries()];
         strncpy(drunkard_walk_features_data_name, root_filename, 4096);

         // read into memory
         fread(drunkard_walk_features, sizeof(RNScalar), data->NBoundaries(), cache_fp);

         value = drunkard_walk_features[this->DataIndex()];
      }
      else {
         random_walk_features = new RNScalar[data->NBoundaries()];
         strncpy(random_walk_features_data_name, root_filename, 4096);

         // read into memory
         fread(random_walk_features, sizeof(RNScalar), data->NBoundaries(), cache_fp);
   ...
   NeuronVoxel *voxel_one = supervoxel_one->CenterVoxel();
   NeuronVoxel *voxel_two = supervoxel_two->CenterVoxel();
...
BoundaryScaledRanking(void) const
{
   ...
   int ncompeting_boundaries = supervoxel_one->NBoundaries() + supervoxel_two->NBoundaries() - 1;
...
CombinedVoxels(void) const
{
   ...
   return (RNScalar)(supervoxel_one->NVoxels() + supervoxel_two->NVoxels());
}
...
CombinedVoxelsProportion(void) const
{
   ...
   if (supervoxel_one->NVoxels() < supervoxel_two->NVoxels())
      return supervoxel_one->NVoxels() / (RNScalar)(supervoxel_one->NVoxels() + supervoxel_two->NVoxels());
   else
      return supervoxel_two->NVoxels() / (RNScalar)(supervoxel_one->NVoxels() + supervoxel_two->NVoxels());
}
...
ReadVoxels(void)
{
   ...
   if (!supervoxel_one->ReadVoxels()) return 0;
   if (!supervoxel_two->ReadVoxels()) return 0;
...
ReadVoxels(FILE *fp, RNBoolean seek)
{
   ...
   if (!supervoxel_one->ReadVoxels(fp, seek)) return 0;
   if (!supervoxel_two->ReadVoxels(fp, seek)) return 0;
...
ReleaseVoxels(void)
{
   ...
   if (!supervoxel_one->ReleaseVoxels()) return 0;
   if (!supervoxel_two->ReleaseVoxels()) return 0;
