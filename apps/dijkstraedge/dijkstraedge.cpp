...
RunDijkstraAlgorithm(void)
{
   ...
   for (int iv = 0; iv < nd->NVoxels(); ++iv) {
      ...
      data[iv].voxel = voxel;
      data[iv].prev = NULL;
      if (voxel->IsOnBoundary()) {
         data[iv].distance = 0.0;
         data[iv].visited = TRUE;
         data[iv].boundary = iv;
         heap.Insert(iv, &(data[iv]));
      }
      else {
   ...
   while (!heap.IsEmpty()) {
      ...
      for (int iv = 0; iv < voxel->NNeighbors(); ++iv) {
         ...
         if (!neighbor_data->visited) {
            ...
            neighbor_data->boundary = current->boundary;
   ...
   // generate arrays
   for (int iv = 0; iv < nd->NVoxels(); ++iv) {
      DijkstraNode *voxel_node = &(data[iv]);
      distances[iv] = voxel_node->distance;
      if (voxel_node->prev) {
         prev_node[iv] = voxel_node->prev->voxel->DataIndex();
      }
      else {
         prev_node[iv] = -1;
      }
      boundaries[iv] = voxel_node->boundary;
   }
...
main(int argc, char **argv)
{
   ...
   *endp = '\0';
   ...
   sprintf(boundaries_filename, "%s/%s_dijkstra_boundaries.distance", distances_directory, root_filename);

   // open file
   FILE *boundary_fp = fopen(boundaries_filename, "wb");
   if (!boundary_fp) { fprintf(stderr, "Failed to write to %s\n", boundaries_filename); exit(-1); }

   // write file
   fwrite(&nvoxels, sizeof(int), 1, boundary_fp);
   fwrite(boundaries, sizeof(int), nvoxels, boundary_fp);
