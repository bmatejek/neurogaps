...
static int show_random_forest_results = 1;
...
static unsigned char *voxel_hash = NULL;
...
static void Draw2D(void)
{
   ...
   for (int iy = 1; iy < selected_slice->YResolution(); ++iy) {
      ...
      for (int ix = 1; ix < selected_slice->XResolution(); ++ix) {
         for (int k = -1; k <= 0; ++k) {
            int iv = nd->IndicesToIndex(ix, iy, selected_slice_index);
            ...
            if (show_projection_human_labels) {
               ...
               if (voxel_hash[iv] == 0 || !show_random_forest_results) {
                  color = Color(selected_slice->GridValue(ix, iy + k), selected_slice_range);
                  color = RNRgb(0.25, 0.25, 0.25) + RNRgb(0.75 * color.R(), 0.75 * color.G(), 0.75 * color.B());
               }
               // true positive
               else if (voxel_hash[iv] == 1) color = RNRgb(0.0, 0.4, 0.0);
               // true negative
               else if (voxel_hash[iv] == 2) color = RNRgb(0.0, 0.0, 0.4);
               // false positive
               else if (voxel_hash[iv] == 3) color = RNRgb(0.4, 0.0, 0.0);
               // false negative
               else if (voxel_hash[iv] == 4) color = RNRgb(1.0, 0.20, 0.0);
            }
            RNLoadRgb(color);
...
void GLUTKeyboard(unsigned char key, int x, int y)
{
   ...
   case 'F':
   case 'f': {
      show_random_forest_results = 1 - show_random_forest_results;
      break; }
...
static int ParseArgs(int argc, char **argv)
{
   ...
   while (argc > 0) {
	 ...
	 else if (!strcmp(*argv, "-global")) global = 1;
...
int main(int argc, char **argv)
{
   ...
   *extp = '\0';
   ...
   char random_forest_filename[4096];
   if (global) sprintf(random_forest_filename, "%s/%s_random_forest_smoothing.binary", graphcut_directory, root_filename);
   else sprintf(random_forest_filename, "%s/%s_random_forest_smoothing_global.binary", graphcut_directory, root_filename);

   // open graphcut file
   FILE *binary_fp = fopen(random_forest_filename, "rb");
   if (!binary_fp) { fprintf(stderr, "Failed to read %s\n", random_forest_filename); exit(-1); }
   ...
   float *binary_terms = new float[nd->NBoundaries()];
   if (fread(binary_terms, sizeof(float), nd->NBoundaries(), binary_fp) != (unsigned int)nd->NBoundaries()) {
   ...
   voxel_hash = new unsigned char[nd->NVoxels()];
   for (int iv = 0; iv < nd->NVoxels(); ++iv)
      voxel_hash[iv] = 0;

   for (int is = 0; is < nd->NSupervoxels(); ++is) {
      if (print_verbose) RNProgressBar(is, nd->NSupervoxels());
      NeuronSupervoxel *supervoxel = nd->Supervoxel(is);
      ...
      for (int iv = 0; iv < supervoxel->NVoxels(); ++iv) {
         ...
         for (int in = 0; in < voxel->NNeighbors(); ++in) {
            ...
            // get supervoxel of neighbor
            NeuronSupervoxel *neighbor_supervoxel = neighbor->Supervoxel();

            // ignore if the voxel and neighbor belong to the same supervoxel
            ...
            for (int ib = 0; ib < supervoxel->NBoundaries(); ++ib) {
               if (supervoxel->Boundary(ib)->OtherSupervoxel(supervoxel) == neighbor_supervoxel)
                  boundary = supervoxel->Boundary(ib);
            }
            rn_assertion(boundary != NULL);

            // random forest output
            int prediction = (int)(binary_terms[boundary->DataIndex()] + 0.5);

            // find out truth
            int human_label = (neighbor_supervoxel->MajorityHumanLabel() == supervoxel->MajorityHumanLabel());

            if (prediction && human_label) true_positive++;
            else if (prediction && !human_label) false_positive++;
            else if (!prediction && human_label) false_negative++;
            else if (!prediction && !human_label) true_negative++;
         ...
         unsigned char max = 0;
         if (true_positive > max_occurrence) {
            max_occurrence = true_positive;
            max = 1;
         }
         if (true_negative > max_occurrence) {
            max_occurrence = true_negative;
            max = 2;
         }
         if (false_positive > max_occurrence) {
            max_occurrence = false_positive;
            max = 3;
         }
         if (false_negative > max_occurrence) {
            max_occurrence = false_negative;
            max = 4;
         }
         voxel_hash[voxel->DataIndex()] = max;
      }
   }
