...
static R3Grid *skeletons_grid = NULL;



// projection variables

static R2Grid *affinity_selected_slice = NULL;
static R2Grid *human_labels_selected_slice = NULL;
static R2Grid *image_selected_slice = NULL;
static R2Grid *skeleton_selected_slice = NULL;
static RNInterval affinity_range;
static RNInterval human_labels_range;
static RNInterval image_range;
static RNInterval skeleton_range;
...
static int show_projection_skeleton = 0;
...
static int show_dimension_skeleton = 0;
...
static void UpdateSlices(void)
{
   ...
   affinity_selected_slice = affinity_grid[RN_X]->Slice(projection_dim, selected_slice_index);
   affinity_range = affinity_selected_slice->Range();
   ...
   skeleton_selected_slice = skeletons_grid->Slice(projection_dim, selected_slice_index);
   skeleton_range = skeleton_selected_slice->Range();

   // update pointers to selected slice
   if (show_projection_affinity) {
      selected_slice = affinity_selected_slice;
      selected_slice_range = affinity_range;
   }
   ...
   else if (show_projection_skeleton) {
      selected_slice = skeleton_selected_slice;
      selected_slice_range = skeleton_range;
   }
}
...
static int ReadVoxelFiles(void) {
   ...
   // read skeleton grid
   skeletons_grid = RNReadNeuronMetaRawFile(skeleton_filename);
   if (!skeletons_grid) { fprintf(stderr, "Failed to read skeleton grid.\n"); exit(-1); }
   skeleton_selected_slice = skeletons_grid->Slice(projection_dim, selected_slice_index);
   skeleton_range = skeleton_selected_slice->Range();
   ...
   delete affinity_grid[RN_Y];
   delete affinity_grid[RN_Z];
   affinity_selected_slice = affinity_grid[RN_X]->Slice(projection_dim, selected_slice_index);
   affinity_range = affinity_selected_slice->Range();
...
static void DrawSlice(void)
{
   ...
   if (show_dimension_affinity) affinity_grid[RN_X]->DrawSlice(projection_dim, selected_slice_index);
   ...
   if (show_dimension_skeleton) skeletons_grid->DrawSlice(projection_dim, selected_slice_index);
   ...
   for (int iy = 1; iy < selected_slice->YResolution(); ++iy) {
      ...
      for (int ix = 1; ix < selected_slice->XResolution(); ++ix) {
         for (int k = -1; k <= 0; ++k) {
            ...
            if (show_overlay) {
               RNScalar skeleton_value = skeleton_selected_slice->GridValue(ix, iy + k);
               if (skeleton_value > 0.5) color = RNred_rgb;
            }
...
void GLUTInit(int *argc, char **argv)
{
   ...
   GLUTwindow = glutCreateWindow("BloodVessel Visualizer");
...
static int ParseArgs(int argc, char **argv)
{
   ...
   while (argc > 0) {
      if ((*argv)[0] == '-') {
         ...
         else if (!skeleton_filename) skeleton_filename = *argv;
         else { fprintf(stderr, "Invalid program argument: %s\n", *argv); return 0; }
      }
      argv++; argc--;
   }
