...
static int
ReadMetaFile(char *input_filename)
{
   // start statistics
   RNTime start_time;
   start_time.Read();
   ...
   *extp = '\0';
   ...
   if (ngrids == 1) {
      grids = new R3Grid *[1];
      grids[0] = RNReadNeuronMetaRawFile(root_filename);
   }
   else {
      grids = RNReadNeuronMetaRawFile(root_filename, TRUE);
   }
...
void GLUTRedraw(void)
{
   ...
   for (int j = ymin; j <= ymax; j++) {
      ...
      for (int i = xmin; i <= xmax; i++) {
         for (int k = -1; k <= 0; k++) {
            // see if boundary or not
            int grid_value = selected_slice->GridValue(i, j + k);

            RNBoolean central_cellular = TRUE;

            // look at all neighbors
            if (i != 0) {
               int right_grid_value = selected_slice->GridValue(i - 1, j + k);
               if (grid_value != right_grid_value) central_cellular = FALSE;
            }
            if (i != selected_slice->XResolution() - 1) {
               int left_grid_value = selected_slice->GridValue(i + 1, j + k);
               if (grid_value != left_grid_value) central_cellular = FALSE;
            }
            if (j + k != selected_slice->YResolution() - 1) {
               int top_grid_value = selected_slice->GridValue(i, j + k + 1);
               if (grid_value != top_grid_value) central_cellular = FALSE;
            }
            if (j + k != 0) {
               int bottom_grid_value = selected_slice->GridValue(i, j + k - 1);
               if (grid_value != bottom_grid_value) central_cellular = FALSE;
            }
            ...
            if (!central_cellular) color = RNblack_rgb;
...
ParseArgs(int argc, char **argv)
{
   ...
   // check filenames
   if (!input_filename) {
      fprintf(stderr, "Usage: metaslice gridfile [text] [options]\n");
      return 0;
   }
...
int main(int argc, char **argv)
{
   ...
   // read grid
   if (!ReadMetaFile(input_filename)) exit(-1);
