...
struct BMBoundaryRank {
   BMBoundaryRank(void) :
      boundary(NULL),
      forest_score(FLT_MAX)
   {}

   BMBoundaryRank(NeuronBoundary *boundary, float forest_score) :
      boundary(boundary),
...
static int MergeResults(void)
{
   ...
   // hash table to see which entries are added to the heap
   ...
   // all boundary cellulars are already TRUE
   for (int ic = 0; ic < nd->NCellulars(); ++ic) {
      if (nd->Supervoxel(ic)->IsOnBoundary()) cellular_hash[ic] = TRUE;
   }

   // get cellular mapping
   cellular_mapping = new int[nd->NCellulars()];
   for (int ic = 0; ic < nd->NCellulars(); ++ic)
      cellular_mapping[ic] = ic;

   // add only boundaries that are on the border
   for (int ib = 0; ib < nd->NBoundaries(); ++ib) {
      NeuronBoundary *boundary = nd->Boundary(ib);

      // skip non boundary supervoxels
      NeuronSupervoxel *supervoxel_one = boundary->SupervoxelOne();
      NeuronSupervoxel *supervoxel_two = boundary->SupervoxelTwo();

      if (!supervoxel_one->IsOnBoundary() && !supervoxel_two->IsOnBoundary()) continue;

      // add this boundary to the heap
      RNScalar smoothing_term = 1.0 - forest_results[boundary->DataIndex()];
      BMBoundaryRank *boundary_rank = new BMBoundaryRank(boundary, smoothing_term);

      // update hash table
      boundary_hash[ib] = TRUE;
      heap.Insert(ib, boundary_rank);
   }

   // go through the queue
   while (!heap.IsEmpty()) {
      // merge the two cellulars if above a certain value
      ...
      // if both already added, do not consider this boundary
      if (cellular_hash[supervoxel_one->DataIndex()] && cellular_hash[supervoxel_two->DataIndex()]) continue;

      // make sure one cellular is added
      rn_assertion(cellular_hash[supervoxel_one->DataIndex()] ^ cellular_hash[supervoxel_two->DataIndex()]);

      // add one cellular with its boundaries
      if (!cellular_hash[supervoxel_one->DataIndex()]) {
         // add all boundaries
         for (int ib = 0; ib < supervoxel_one->NBoundaries(); ++ib) {
            NeuronBoundary *boundary = supervoxel_one->Boundary(ib);
            if (boundary_hash[boundary->DataIndex()]) continue;

            if (boundary->OtherSupervoxel(supervoxel_one)->IsExtracellular()) continue;
            ...
            // update hash table
            boundary_hash[boundary->DataIndex()] = TRUE;
            heap.Insert(boundary->DataIndex(), boundary_rank);
         }

         cellular_hash[supervoxel_one->DataIndex()] = TRUE;

         cellular_mapping[supervoxel_one->DataIndex()] = cellular_mapping[supervoxel_two->DataIndex()];
      }
      if (!cellular_hash[supervoxel_two->DataIndex()]) {
         // add all boundaries
         for (int ib = 0; ib < supervoxel_two->NBoundaries(); ++ib) {
            NeuronBoundary *boundary = supervoxel_two->Boundary(ib);
            if (boundary_hash[boundary->DataIndex()]) continue;

            if (boundary->OtherSupervoxel(supervoxel_two)->IsExtracellular()) continue;
         ...
         cellular_hash[supervoxel_two->DataIndex()] = TRUE;

         cellular_mapping[supervoxel_two->DataIndex()] = cellular_mapping[supervoxel_one->DataIndex()];
      }
   }
...
int main(int argc, char **argv)
{
   ...
   *extp = '\0';
   ...
   if (merge_validation) {
      ...
      sprintf(input_result_filename, "%s/%s.results", results_directory, root_filename);
      ...
      // go through all boundarymerge results
      for (int it = 0; it <= 100; ++it) {
         RNProgressBar(it, 101);

         // get boundary filename
         char boundaries_filename[4096];
         sprintf(boundaries_filename, "%s/%s_boundarymerge_%03d.boundary", boundaries_directory, root_filename, it);
      ...
      sprintf(proportion_filename, "%s/%s_boundarymerge_proportion.feature", features_directory, root_filename);
      sprintf(first_split_filename, "%s/%s_boundarymerge_first_split.feature", features_directory, root_filename);
      sprintf(last_split_filename, "%s/%s_boundarymerge_last_split.feature", features_directory, root_filename);
      ...
      char output_filename[4096];
      sprintf(output_filename, "%s/%s.results", results_directory, root_filename);
